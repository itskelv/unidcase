...
class SELDLossADPIT(nn.Module):
    ...
    def _mse(self, output, target):
        num_frame = min(output.shape[1], target.shape[1])
        output = output[:, :num_frame, ...]
        target = target[:, :num_frame, ...]
    ...
    def forward(self, output, target):
        ...
        num_frame_out = output.shape[1]
        num_frame_tgt = target.shape[1]
        num_frame = min(num_frame_out, num_frame_tgt)
        num_track = 3
