...
This module defines evaluation metrics for SELD (Sound Event Localization and Detection) tasks.
It implements both class-sensitive localization and location-sensitive detection metrics.

Classes:
    SELDMetrics: Main class for computing SELD evaluation metrics
    ComputeSELDResults: Utility class for computing SELD results from prediction files

Key Features:
    - Location-sensitive detection metrics (F-score)
    - Class-sensitive localization metrics (angular error, distance error)
    - Support for both micro and macro averaging
    - Jackknife confidence interval estimation
    - Multi-track sound event evaluation
    - Onscreen accuracy evaluation for audio-visual models

Metrics:
    - F-score: Location-sensitive detection performance
    - Angular Error: Average angular error in degrees
    - Distance Error: Average distance error in meters
    - Relative Distance Error: Average relative distance error
    - Onscreen Accuracy: Percentage of correct onscreen predictions
...
class ComputeSELDResults:
    def __init__(self, params=None, ref_files_folder=None):
        """
        Initialize evaluation metrics calculator
        
        Args:
            params (dict): Parameter dictionary, if None attempts to import from parameters
            ref_files_folder (str): Reference files directory
        """
        # If no parameters passed, try to import from parameters
        if params is None:
            try:
                ...
                raise ImportError("Parameter configuration not found. Please ensure:\n"
                                "1. Parameters are provided through configuration files in experiments directory, or\n"
                                "2. A parameters.py file exists in the root directory as default configuration")
        else:
            self.params = params
            
        self.ref_files_folder = ref_files_folder

        self._desc_dir = ref_files_folder if ref_files_folder is not None else os.path.join(self.params['root_dir'],
                                                                                            'metadata_dev')
        self._doa_thresh = self.params['lad_doa_thresh']
        self._dist_thresh = self.params['lad_dist_thresh']
        self._reldist_thresh = self.params['lad_reldist_thresh']
        self._req_onscreen = self.params['lad_req_onscreen']

        if self.params['modality'] == 'audio' and self.params['lad_req_onscreen']:
        ...
        self._average = self.params['average']
        self._nb_classes = self.params['nb_classes']
...
if __name__ == '__main__':
    ...
    try:
        ...
        print("Warning: parameters.py not found, please provide parameters through configuration files in experiments directory")
        exit(1)
    ...
    use_jackknife = False
    eval_dist = test_params['evaluate_distance'] if 'evaluate_distance' in test_params else False
    score_obj = ComputeSELDResults(test_params, ref_files_folder='../DCASE2025_SELD_dataset/metadata_simple_header_int_dev')
